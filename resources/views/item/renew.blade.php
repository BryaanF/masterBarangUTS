...
<div class="container-sm my-5">
    <form action="{{ route('item.update', ['item' => $items->id]) }}" method="POST">
        @csrf
        @method('put')
        <div class="row justify-content-center">
            ...
            is-invalid @enderror" type="text"
                            name="name" id="name" value="{{ $items->name }}" placeholder="Enter the name">
                        @error('name')
            ...
            @enderror" type="number"
                            name="price" id="price" value="{{ $items->price }}" placeholder="Enter the price">
                        @error('price')
                            <div class="text-danger"><small>{{ $message }}</small></div>
                        @enderror
                    </div>
            ...
            is-invalid @enderror"
                            type="text" name="description" id="description" value="{{ $items->description }}"
                        ...
                        <select name="unit" id="unit" class="form-select">
                            @foreach ($units as $unit)
                                <option value="{{ $unit->id }}"
                                    {{ $items->unit_id == $unit->id ? 'selected' : '' }}>
